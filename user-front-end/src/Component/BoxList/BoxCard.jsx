...
const BoxCard = (props) => {
    ...
    return (
        <div className="media menu-item p-2" onClick={()=> {nav(`/boxDetails?bid=${props.box._id}`)}} style={{ backgroundColor: "white" }}>
            <img className="mr-3 img-fluid" src={"http://localhost:2020/Images/Boxes/"+props.box.BImageURL} style={{ width: 200, height: 130, borderRadius: 5 }} alt={props.box.BImageURL} />
            <div className="media-body">
                <h5 className="mt-0">{props.box.BName}</h5>
                <p>{props.box.BDescription}</p>
                <p className='location' style={{marginTop: -15}}>{props.box.BCity}, {props.box.BArea} </p>
